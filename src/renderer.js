...
export async function renderVideo(options = {}) {
  ...
  // Enforce even pixel dimensions required by hardware encoders
  ...
  const extMap = { mp4: '.mp4', webm: '.webm', prores: '.mov', 'png-sequence': '.zip' };
  ...
  try {
    ...
    const animationMeta = await page.evaluate(async ({ jsCode, plan, forcedDuration }) => {
      ...
      return { duration: computedDuration, tweensCount: children.length };
    }, { jsCode: jsContent, plan: motionPlan, forcedDuration: options.duration });
    ...
    for (let frameIndex = 0; frameIndex < totalFrames; frameIndex++) {
      ...
      await page.evaluate((currentTimeSec) => {
        ...
        if (typeof document.getAnimations === 'function') {
          document.getAnimations().forEach((anim) => { anim.currentTime = currentTimeSec * 1000; });
        }
